...
Content:
  ...
  --brand=auto|primary|stacked|both
      auto    = [+] USEFUL/STASH everywhere
      primary = [+] USEFUL/STASH
      stacked = USEFUL / STASH_
      both    = render both primary and stacked variants for every preset
      Default: auto
...
for arg in "$@"; do
  case "$arg" in
    --preset=*) PRESETS[PRESET_COUNT]="${arg#*=}"; PRESET_COUNT=$((PRESET_COUNT + 1)) ;;
    --all) append_group "${ALL_PRESETS[@]}" ;;
    --vertical) append_group "${VERTICAL_PRESETS[@]}" ;;
    --social) append_group "${SOCIAL_PRESETS[@]}" ;;
    --tag=*) TAGS[TAG_COUNT]="${arg#*=}"; TAG_COUNT=$((TAG_COUNT + 1)) ;;
    --title=*) TITLE="${arg#*=}" ;;
...
case "$BRAND" in
  auto|primary|stacked|both) ;;
  *)
    echo "Error: --brand must be auto, primary, stacked, or both." >&2
    exit 1
    ;;
esac
...
if (( PRESET_COUNT == 0 )); then
  PRESETS[0]="instagram-square"
  PRESET_COUNT=1
fi

# Validate + de-duplicate without associative arrays so this works on macOS Bash 3.2.
# Important: Bash 3.2 + `set -u` can treat expansion of an empty array as an
# unbound variable, so only expand UNIQUE_PRESETS after it has at least one item.
UNIQUE_PRESETS=()
UNIQUE_PRESET_COUNT=0
...
for preset in "${PRESETS[@]}"; do
  ...
  if (( seen == 0 )); then
    UNIQUE_PRESETS[UNIQUE_PRESET_COUNT]="$preset"
    UNIQUE_PRESET_COUNT=$((UNIQUE_PRESET_COUNT + 1))
  fi
done
...
def render_brand(preset_name, x, y, primary_size, stacked_size, brand_mode):
    brand = brand_mode
...
def layout_profile(preset_name, width, height, layout):
    ...
    if layout == "portrait":
        if preset_name == "threads":
            ...
            # Instagram 4:5 feed artwork: larger brand and tags,
            # without letting the metadata overpower the headline.
            brand_size = 40
            stacked_size = 64
            tag_size = 36
            subtitle_size = 40
    ...
    return {
        ...
        # On 9:16 artwork the lockup and tags must survive phone-scale viewing.
        "brand_size": 50,
        "stacked_size": 74,
...
def render(preset_name, brand_mode):
...
if brand_option == "both":
    brand_variants = [
        ("primary", "-primary"),
        ("stacked", "-stacked"),
    ]
else:
    brand_variants = [(brand_option, "")]

for preset_name in selected_presets:
    for brand_mode, suffix in brand_variants:
        width, height, svg = render(preset_name, brand_mode)
        path = output_dir / f"{preset_name}{suffix}.svg"
...
EOF
  ...
  for preset in "${UNIQUE_PRESETS[@]}"; do
    ...
    if [[ "$BRAND" == "both" ]]; then
      BRAND_SUFFIXES=("primary" "stacked")
    else
      BRAND_SUFFIXES=("")
    fi

    for brand_suffix in "${BRAND_SUFFIXES[@]}"; do
      if [[ -n "$brand_suffix" ]]; then
        SVG="${OUTPUT}/${preset}-${brand_suffix}.svg"
        PNG="${OUTPUT}/${preset}-${brand_suffix}.png"
      else
...
if (( MAKE_SVG == 0 )); then
  for preset in "${UNIQUE_PRESETS[@]}"; do
    if [[ "$BRAND" == "both" ]]; then
      rm -f "${OUTPUT}/${preset}-primary.svg" "${OUTPUT}/${preset}-stacked.svg"
    else
